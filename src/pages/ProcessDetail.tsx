            ...
            {CUSTOMER_AGREEMENT_STEP_KEYS.includes(selectedKey) && (
              <div className="mt-6 rounded-xl border border-border bg-background/40 p-4">
                <TodoList
                  ...
                  onAdd={(title) => addCT(process.id, title)}
                  onRemove={(id) => removeCT(process.id, id)}
                  onToggle={(id) => toggleCT(process.id, id)}
                  onChangeDueDate={(id, dueDate) => setCTDue(process.id, id, dueDate)}
...
const TradeInFields = ({ fields, onChange, disabled }: { fields: ProcessFields; onChange: (patch: Partial<ProcessFields>) => void; disabled?: boolean }) => (
